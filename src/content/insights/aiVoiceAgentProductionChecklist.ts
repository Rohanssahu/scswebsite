 ...
 *
 * Written from first-hand experience of building and running Buddy — the
 * LiveKit-based voice agent that answers on `/schedule-call` and inside the
 * consultation rooms. The failure modes listed are ones we hit ourselves.
 *
 * Deliberately absent: prompt text, provider credentials, model names tied to a
 * specific contract, client data, call volumes, any capability we have not
 * shipped, and any suggestion that our own agent is a client case study.
 */

import { insightContent } from './compose';
import type { InsightBody } from './types';

const body: InsightBody = {
  path: '/insights/ai-voice-agent-production-checklist',
  h1: 'Production checklist for an AI voice agent',
  standfirst:
  ...
  basis:
    'We build and operate our own voice agent: it answers on our scheduling page and joins consultation rooms over LiveKit, running a speech-to-text → language model → text-to-speech pipeline. Every failure mode below is one we hit ourselves, in that order, roughly.',
  readingMinutes: 11,

  sections: [
    {
      id: 'latency-budget',
      heading: 'Have a latency budget, not a latency hope',
      body: [
      ...
      note: 'Retrieval belongs inside this budget too. A lookup that takes 800 ms is invisible in a chat interface and fatal in a conversation.',
    },
    {
      id: 'interruption',
      heading: 'Interruption is a feature, and it is harder than it sounds',
      body: [
    ...
    },
    {
      ...
      heading: 'Plan for silence, noise and the things people actually say',
      body: [
      ...
      note: 'For anything an error would be expensive on — a reference number, a spelled-out email — read it back and confirm. That is a design decision, not a model capability.',
    },
    {
      id: 'tools-and-actions',
      heading: 'Decide what the agent may do, not just what it may say',
      body: [
    ...
    },
    {
      ...
      heading: 'Consent and privacy, decided before launch rather than after',
      body: [
        'Voice is personal data, and a recording is more sensitive than a transcript. The rules differ by jurisdiction and we do not give legal advice — but every deployment has to make the same set of decisions explicitly, and the time to make them is before the first real call, not after a complaint.',
      ],
      points: [
        'Tell the caller they are speaking to an automated system, at the start, in plain words. This is increasingly a legal requirement and it is good practice regardless.',
        'Decide whether audio is retained at all. Retaining transcripts and discarding audio is often enough for quality work and is a much smaller exposure.',
        'Set a retention period and enforce it automatically. A policy that depends on someone remembering to delete is not a retention period.',
        'Redact identifiers from transcripts before they reach any log, dashboard or analytics tool.',
        'Have an escalation path to a human, and make it easy to reach. "Talk to a person" should always work.',
        'Confirm with the client where audio and transcripts are processed and stored, and put it in writing. Do not claim a certification you do not hold.',
      ],
      note: 'We treat this as a scoping question with the client rather than a technical default, because the honest answer depends on their sector and their own legal advice.',
    },
    {
      id: 'fallback',
      heading: 'Design the failure path as carefully as the happy path',
      body: [
    ...
    {
      id: 'monitoring',
      heading: 'Monitor conversations, not just uptime',
      body: [
      ...
      note: 'Every one of these can be collected without storing what an individual caller said. Aggregate signals first; keep transcripts only where there is a stated reason and a retention period.',
    },
    {
      id: 'before-launch',
      heading: 'The pre-launch list',
      body: [
      ...
      note: 'The last one gets forgotten most often and matters most. If the agent starts behaving badly on a Friday evening, someone needs a switch, not a release process.',
    },
  ],

  faqs: [
    {
      question: 'How fast does a voice agent actually need to respond?',
      answer:
    ...
    {
      question: 'Why does the agent talk over people, or refuse to stop?',
      answer:
    ...
    {
      question: 'Should a voice agent be allowed to book, cancel or refund?',
      answer:
    ...
    {
      question: 'Do we have to record calls?',
      answer:
    ...
    {
      question: 'What happens when the model or the speech service goes down mid-call?',
      answer:
    ...
    {
      question: 'How do you tell whether the agent is doing a good job?',
      answer:
    ...
    {
      question: 'Can an existing phone system be kept?',
      answer:
  ...
  related: [
    {
      ...
      blurb: 'What we build, what we do not claim, and how human review sits over the agent’s output.',
    },
    {
      path: '/services/ai-video-consultation-agents',
      label: 'AI Video Consultation Agents',
      blurb: 'The same pipeline with a video surface, used for scheduled consultations rather than calls.',
    },
    {
      path: '/services/conversational-ai-development',
      label: 'Conversational AI Development',
      blurb: 'When text is the right channel, and how escalation to a person is designed.',
    },
  ],

  cta: {
    title: 'Talk to the agent we built',
    body: 'Our own consultation agent runs the pipeline described here. Start a session and see how it handles interruption, silence and a handover to a person — then tell us what you are trying to build.',
  },
};

export const aiVoiceAgentProductionChecklist = insightContent(body);
export default aiVoiceAgentProductionChecklist;
