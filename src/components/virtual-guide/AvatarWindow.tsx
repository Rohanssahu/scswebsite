...
// side — right in LTR, left in RTL. Full screen (header button, or a header
// double-click on desktop) makes the panel fill the viewport, with the body
// centred so long chats stay readable on wide monitors.
...
const AvatarWindow = ({ guide, buddy, isMobile }: AvatarWindowProps) => {
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Escape') {
      e.stopPropagation();
      // Escape leaves full screen first, so it never closes the chat by surprise.
      if (expanded) guide.toggleExpand();
      else guide.closePanel();
    }
  };

  // Full screen centres the body; the docked panel is already narrow.
  const bodyWidth = expanded ? 'mx-auto w-full max-w-3xl' : 'w-full';
  ...
  const panel = (
    <motion.div
      ...
      animate={expanded ? { opacity: 1, x: 0, y: 0 } : { opacity: 1, y: 0 }}
      exit={guide.reduceMotion ? undefined : { opacity: 0, y: 24 }}
      transition={{ duration: 0.2 }}
      className={
        expanded
          ? 'pointer-events-auto fixed inset-0 z-[80] flex h-[100dvh] w-full flex-col overflow-hidden bg-white pb-[env(safe-area-inset-bottom)] pt-[env(safe-area-inset-top)] outline-none'
          : isMobile
    ...
    >
      ...
      <div
        ...
        className={`flex items-center justify-between gap-2 bg-gradient-to-r from-orange-500 via-pink-500 to-purple-600 px-3 py-2.5 ${
          isMobile || expanded ? '' : 'cursor-grab active:cursor-grabbing'
        }`}
      >
        <div className="flex min-w-0 items-center gap-2">
          {!isMobile && !expanded && <GripHorizontal className="h-4 w-4 shrink-0 text-white/60" aria-hidden="true" />}
        ...
        <GuideControls guide={guide} compact={!isMobile && !expanded} />
      </div>

      {/* Avatar stage with live captions */}
      <div className="relative shrink-0 bg-gradient-to-br from-gray-900 via-purple-950 to-gray-900 px-4 py-3">
        <div className={`flex items-center gap-3 ${bodyWidth}`}>
          <div className="shrink-0">
            <GuideAvatar
              state={guide.avatarState}
              size={expanded ? (isMobile ? 80 : 104) : isMobile ? 64 : 80}
              reduceMotion={guide.reduceMotion}
              level={guide.voiceLevel}
            />
          </div>
          <div className="min-w-0 flex-1">
            ...
            <p
              className={`mt-1 line-clamp-3 leading-relaxed text-gray-200 ${expanded ? 'text-sm' : 'text-xs'}`}
              aria-live="polite"
              aria-label={t('guide.captionsLabel')}
            >
      ...
      <div className={`flex min-h-0 flex-1 flex-col ${bodyWidth}`}>
