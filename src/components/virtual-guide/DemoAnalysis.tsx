...
const DemoAnalysis = ({ onComplete, reduceMotion }: DemoAnalysisProps) => {
  ...
  return (
    <div
      className="max-w-[92%] rounded-2xl rounded-ss-sm border border-gray-200 bg-white px-3.5 py-3"
      role="status"
      aria-live="polite"
      aria-label={t('guide.analysis.inProgress')}
    >
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-gray-900">{t('guide.analysis.title')}</p>
        <span className="rounded-full border border-amber-300 bg-amber-50 px-2 py-0.5 text-[10px] font-medium text-amber-700">
          {t('guide.analysis.badge')}
        </span>
      </div>
