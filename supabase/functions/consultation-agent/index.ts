...
function proposalRow(meetingId: string, version: number, p: ValidatedProposal): Record<string, unknown> {
  return {
    ...
    // The budget-fit snapshot rides inside the existing `proposal` jsonb column
    // (no schema change): included/deferred scope, the two optional tiers, the
    // coverage percentage and the provider metadata the admin team reviews.
    proposal: { ...p.content, budget_plan: p.budget_plan },
        ...
        const payload = {
          ...
          requirement: {
            ...
            demo_estimate: {
              ...
              hourly_rate_usd: d.proposal.content.hourly_rate_max,
              ...
              budget_plan: d.proposal.budget_plan,
              client_selected_option: d.proposal.budget_plan?.client_selected_option ?? null,
              human_review_required: true,
            },
            estimate_version: (d.proposal.budget_plan?.estimate_version ?? d.proposal.config_version).slice(0, 40),
          ...
          proposal: {
            ...
            proposal: { ...d.proposal.content, budget_plan: d.proposal.budget_plan },
            total_hours_min: d.proposal.total_hours_min,
            total_hours_max: d.proposal.total_hours_max,
            total_cost_min: d.proposal.total_cost_min,
            total_cost_max: d.proposal.total_cost_max,
